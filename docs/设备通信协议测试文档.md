...
| 步骤2 | `POST /device/query-status` | code=0, **delivery_method=realtime** |
...
| heartbeat_report | T6, T8, **T12** | 无命令/有命令/**WebSocket心跳** |
| server_ack | T1~T10, **T12, T13** | 所有上报接口均返回 |
| time_sync (首次上报) | T1 | **first_report_at 为 NULL 时触发** |
| time_sync (心跳) | T6, T8, **T12** | 收到心跳后触发 |
| **WebSocket 长连接** | **T12, T13** | **⭐ 双向长连接通信（v3.0 新增）** |
| **WebSocket 命令推送** | **T13** | **⭐ 通过 WS 实时推送 query_device_status** |
| query_device_status (实时-WS) | **T13** | **WebSocket 实时推送（优先级最高）** |
| query_device_status (实时-LP) | **T7** | **长轮询 + asyncio.Queue 实时推送** |
| query_device_status (离线) | **T8** | **命令排队 + 心跳获取（向下兼容）** |
| query_device_status (admin) | T9 | 管理后台触发，返回 delivery_method |
| 长轮询监听 | **T7** | **GET /device/listen/{id} 设备实时监听** |
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  T7     实时查询-长轮询(完整流程)        ✅ PASS  监听✅、下发LP✅、实时收到✅、上报✅
  T8     离线回退(心跳携带)               ✅ PASS  delivery=queued, 心跳携带命令
  T9     管理后台主动查询                 ✅ PASS  delivery_method=websocket/long_polling/queued
  T10    错误校验码上报                   ✅ PASS
  T11    扫码上报(报文格式演示)           ✅ PASS  仅格式演示
  T12    WebSocket连接+心跳 ⭐           ✅ PASS  连接✅、心跳✅、ack✅、time_sync✅
  T13    WebSocket命令实时推送 ⭐         ✅ PASS  WS连接✅、delivery=websocket✅、命令收到✅
```
...
| **T12 跳过** | **`websockets` 库未安装** | **执行 `pip install websockets`** |
| **T13 跳过** | **`websockets` 库未安装** | **执行 `pip install websockets`** |
